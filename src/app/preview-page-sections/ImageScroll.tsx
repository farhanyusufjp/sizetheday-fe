...
const DivSlider = () => {
  ...
  const botttomDivX = useTransform(scrollYProgress, [animationStart, 1], [0, 100]);
    ...
    >
      ...
      >
        {templateDetails.screenshotsOne.map((screenshot) => (
          <div key={screenshot.src} className='min-w-[30%] overflow-hidden rounded-xl shadow-lg'>
            <Image src={screenshot} alt='screenshot' className='w-full object-cover' />
          </div>
        ))}
      </motion.div>
      <motion.div
