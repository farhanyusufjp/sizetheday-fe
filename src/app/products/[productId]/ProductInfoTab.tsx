...
const ProductInfoTab: FC<ProductInfoTabProps> = ({ overview, shipment_details }) => {
  ...
  return (
    <div>
      ...
      <div className='mb-10 flex items-center gap-5'>
        {tabs.map((tab) => (
          <button
            ...
            className={`py-4 ${
              activeTab === tab ? 'border-b-2 border-primary' : 'text-neutral-500'
            }`}
          >
            {tab}
          </button>
      ...
      {tabs.map((tab) => (
        <div key={tab} className={`mb-10 ${activeTab === tab ? 'block' : 'hidden'}`}>
          {activeTab === 'Overview' ? (
            <p>{overview}</p>
          ) : (
