...
const BlogCard: FC<BlogCardProps> = ({ title, brief, date, coverImage, tag, slug }) => {
  return (
    <motion.div
    ...
    >
      <Link href={`/blog/${slug}`} className='overlay absolute inset-0 z-20 size-full' />
      ...
      <div className='absolute bottom-5 left-0 z-40 w-full px-5'>
        ...
        <Link href={`/blog/${slug}`} className='card-title text-2xl font-semibold'>
          {title}
        </Link>
